...
function pwckeck() {
	...
	if(f.PWD.value != ${dto.pwd}){
		...
		f.action = "<%=cp%>/itving/customUpdate.do";
		f.submit();
				...
				<ul>
					...
					<li class="login_member">
						<a href="<%=cp %>/itving/myPage.do">
							<font color="pink">${sessionScope.customInfo.name }</font> 님 환영합니다.
						</a>
    ...
    <div class="sub-contents-wrap inner-wrap">
        <form action="/my/user/confirmPasswordProc.do" name="frmChk" id="frmChk" method="post">
            ...
            <section class="coupon-regist-box">
                <h4>회원 정보 수정을 위해 비밀번호를 입력해주세요.</h4>
                <p>회원님의 개인 정보 보호를 위해 비밀번호를 확인합니다.</p>

                <div class="form-box">
                    <input type="password" id="PWD" placeholder="비밀번호">
                    <input type="button"onclick="pwckeck();" value="확인" style="color: black;">
                </div>
            </section> 
        </form>
    </div>
