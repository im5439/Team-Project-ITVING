...
<body data-n-head="">
	...
	<div id="wrap" class="page-live">
		...
		<header id="header" class="page-live-main">
			...
			<script id="search-keyword_Template"
				type="text/x-handlebars-template">
<span class="fl">실시간 인기 검색어</span>';
<span class="fr">{{recentDisplay}}</span>';
<ul class="keyword-list">
	{{#each keywordList}}
	
	{{#if isTop}}
		<li class="top"><a href="#" class="tit node" title="'{{keyword}}">{{index}} . {{keyword}}</a></li>
	{{^}}
		<li><a href="#" class="tit node" title="{{keyword}}">{{index}} . {{keyword}}</a></li>
	{{/if}}
	{{/each}}
</ul>
</script>

			<script id="search-keyword-box_Template"
				type="text/x-handlebars-template">
{{#each keywordList}}
	<div class="keyword-box">
		<div class="keyword-lnk">
			<p class="title"><strong class="key">data.ch_nm</strong></p>
			<a href="'+ url + data.ch_cd +'" title="data.mast_nm" class="node">채널시청 바로가기</a>
		</div>';
		<div class="channel-logo"><span class="ch_data.ch_cd">data.ch_nm</span></div>
		<div class="thumb"><img src="#imageDomain data.web_url3" alt="data.ch_nm"></div>
	</div>			
{{/each}}
		...
		<div class="inner-live-main container">
			...
			<div id="EXCD0183">
				<section class="lists live-lists">
					<h2 class="section-title">실시간 인기 채널</h2>
						...
						<div class="bx-viewport" aria-live="polite"
							...
							<div id="live-home-top-channel" class="bxslider bxslider-in"
								...
								<div class="lists-wrap"
									style="float: left; list-style: none; position: relative; width: 1224px; margin-right: 24px;"
									aria-hidden="false">

									<c:set var="i" value="1"/>
									<c:forEach var="dto" items="${lists }">
										<div class="reallive-item column-3">
											<a href="<%=cp %>/itving/playVideo.do?vnum=${dto.vnum }" title="tvN">
												<div class="reallive-item__image">
													<img src="${imagePath }/${dto.ilink }" width="640" heigth="360"/>
												</div>
												<div class="reallive-item__info">
													<p class="reallive-item__info-title">${dto.vtitle }</p>
													<em class="reallive-item__info-rank">${i }</em>
												</div>
											</a>
										</div>
					...
					<div class="btn-more">
						<a href="<%=cp %>/itving/videoList.do">전체보기</a>
					</div>
				</section>
			</div>
