			...
			<div class="sub-contents-wrap inner-wrap">
				...
				<div class="tab_type01">
					<ul>
						...
						<small>|</small>
						<li><a href="<%=cp%>/itving/myBoard.do" class="on">내 글
								보기</a></li>
						<small>|</small>
				...
				<div class="tab-second-block cb">
					...
					<div id="EXCD0185">
						<section class="lists">
						<h2 class="section-title">내 글 보기</h2>
						<table class="tbl-list normal">
							...
							<tbody id="contents-list">
								<c:forEach var="rdto" items="${mblists }">
									<tr>
										<td style="font-size: 10pt;"><span>${rdto.re_num }</span></td>
										<td class="title" style="font-size: 10pt;" align="center">
											<a class="bold" href="${articleURL }&re_num=${rdto.re_num }">
												[&nbsp; <c:if test="${rdto.category=='game'}">게임</c:if> <c:if
													test="${rdto.category=='drama'}">드라마</c:if> <c:if
													test="${rdto.category=='etc'}">기타</c:if> <c:if
													test="${rdto.category=='sports'}">스포츠</c:if> <c:if
													test="${rdto.category=='entertainment'}">예능</c:if> <c:if
													test="${rdto.category=='movie'}">영화</c:if> <c:if
													test="${rdto.category=='notice'}">공지</c:if> &nbsp;]
												&nbsp;&nbsp; ${rdto.subject }
										</a> <c:if test="${rdto.dat_count >0 }">(${rdto.dat_count })</c:if>
										</td>
