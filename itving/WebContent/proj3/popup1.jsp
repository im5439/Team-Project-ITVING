...
<script language="javascript">
  function pay() { window.open("<%=cp%>/itving/pay1.do", "a", "width=500, height=700, left=100, top=50 scrollbars = yes" ); }
  </script>
...
<!-- 480 X 670 -->
	<div id="wrap">
		<div id="header">
			<h1 class="single">구매하기</h1>
		</div>
				<div class="pack_ch">
			<dl>
				<dt id="purchaseProdName" title="아이티빙 무제한">아이아이티빙 무제한 정기권</dt>
				<dd>
                                            PC, 스마트폰, 태블릿, 크롬캐스트에서 아이티빙의 모든 방송과 월정액 무제한 영화관 내 Lite 영화 500여편을 시청할 수 있는 이용권입니다. 고화질(720P)로 아이티빙의 모든 콘텐츠를 즐겨보세요. (일부 VOD 제외)
                    				</dd>
			</dl>
		</div>
		...
		<div id="contents">
            ...
            <div class="amount">
				<span>결제할 금액 <strong>9,900</strong> 원
				</span> <em>(VAT포함)</em>
				<p>매월 자동결제</p>
			</div>
	...
	<ul>
			<li>
			<input type="radio" name="paySeq" value="29" paytoolcode="mobile" id="paytool_mobile" checked="checked">
			<label id="label_mobile" for="paytool_mobile"><strong>
				무통장 입금
			</strong></label>
		</li>
		</ul>
	<p>
		<input type="checkbox" id="check_trem_pay" name="check_term"> 
		            <label for="check_trem_pay">위 내용을 확인하였으며,
                <span id="spanRecurpayTxt" style="display:show;"> 월 자동결제 및</span>
				<a href="#" onclick="alertLayerShowById('legal_charge_layer'); return true;">유료 이용약관
				</a>에 동의합니다.
			</label>
    ...
    <!-- tier theme만 노출 -->
        ...
        <dd>
			...
			- 스마트TV 권장사양 : LG 스마트TV 2015년형 이상 모델(web OS 2.0 이상) / 삼성 스마트TV 2016년형 이상 모델(타이젠 2.4 이상)<br>
					...
					<div class="act_area">
						<a href="<%=cp %>/itving/pay1.do" onclick="pay();" class="btn next" id="payBtn">다음</a> 
						<a href="" onclick="window.close()" class="btn cancel" id="cancelBtn">취소</a>
					</div>
									</fieldset>
				<input type="hidden" name="PRODID" value="1204827">
				<input type="hidden" name="PRODTYPE" value="2">
				<input type="hidden" name="CHARGEAMT" value="5900">
...
<script type="text/javascript">
    ...
    $( window ).load( function() {
        ...
        $( "#preBtn" ).click( function() {
            ...
            if ( "1204827" != "" ) {
                //상품
									preUrl += "/recurpay_page.tving?PRODID=1204827&PRODTYPE=2";
        ...
        $( ".layer" ).find( "#purchaseProdName" ).text( billingPc.displayProdName( "아이티빙 무제한", "2", "", "30", "3", "", "" ) );

        $( "div" ).find( "#purchaseProdName" ).text( billingPc.displayProdName( "아이티빙 무제한", "2", "", "30", "3", "", "" ) );
